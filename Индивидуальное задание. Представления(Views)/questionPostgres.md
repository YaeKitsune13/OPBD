...
| `policyholder_name` | ФИО страхователя                      |
...
| `employee_name`     | ФИО сотрудника, оформившего полис     |
...
![Кратковременное страхование](Изображения/short_term_insurance_view_postgres.png)

**Объяснение:**

- В представлении отображаются только полисы, у которых `duration_days ≤ 14`
- На��ример, полис на 7 дней для туристической поездки
- Автоматически фильтруются записи при добавлении новых полисов
...
| `insurance_type_id` | ID вида страхования         | -                                   |
| `insurance_type`    | Вид страхования             | -                                   |
| `year`              | Год заключения договоров    | `EXTRACT(YEAR FROM contract_date)`  |
...
![Доходы по видам страхования](Изображения/revenue_view_postgres.png)
...
![Страховые выплаты](Изображения/payouts_view_postgres.png)
...
> В PostgreSQL можно выполнять INSERT через представления, **если представление основано на одной таблице** и не содержит агрегатных функций, GROUP BY, DISTINCT и т.д.
...
![INSERT через представление](Изображения/insert_view_postgres.png)
...
```sql
-- Обновляем ��тоимость полиса через представление
...
![UPDATE через представление](Изображения/update_view_postgres.png)
...
![DELETE через представление](Изображения/delete_view_postgres.png)
