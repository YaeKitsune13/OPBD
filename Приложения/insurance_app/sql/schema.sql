...
create trigger trg_policyholders_after_insert
...
create trigger trg_policyholders_after_update
after update on policyholders
for each row
begin
    if new.employee_id <> old.employee_id then
        update employees
           set count_policyholders = greatest(coalesce(count_policyholders, 0) - 1, 0)
...
create trigger trg_policyholders_after_delete
after delete on policyholders
for each row
begin
    update employees
       set count_policyholders = greatest(coalesce(count_policyholders, 0) - 1, 0)
