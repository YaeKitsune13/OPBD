...
class DatabaseSection(QWidget):
    ...
    def init_ui(self, title):
        ...
        # Заголовок секции (MySQL / PostgreSQL)
        lbl_title = QLabel(title)
        lbl_title.setFont(font_title)
        layout.addWidget(lbl_title)

        # Подзаголовок 1
        lbl_sub1 = QLabel("1: Таблица выделяет Рабочих с помощью SQL запроса")
        lbl_sub1.setFont(font_subtitle)
        layout.addWidget(lbl_sub1)

        # Таблица сотрудников (Master)
        self.master_view = QTableView()
        self.master_view.setSelectionBehavior(QTableView.SelectionBehavior.SelectRows)
        self.master_view.setSelectionMode(QTableView.SelectionMode.SingleSelection)
        self.master_view.clicked.connect(self.on_employee_selected)
        ...
        self.detail_view_safe = QTableView() # Левая таблица
        self.detail_view_safe.setSelectionBehavior(QTableView.SelectionBehavior.SelectRows)
        
        self.detail_view_raw = QTableView()  # Правая таблица
    ...
    def load_employees(self):
        """ Загрузка списка сотрудников (Верхняя таблица) """
        db = QSqlDatabase.database(self.db_name)
        query = QSqlQuery(db)
        query.exec(SQL_EMPLOYEES)
        
        self.master_model.setQuery(query)
        self._set_employee_headers(self.master_model)
        self.master_view.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)

    def on_employee_selected(self, index):
        """ Обработка клика по сотруднику """
        # Получаем ID сотрудника (Колонка 0)
        employee_id = self.master_model.data(self.master_model.index(index.row(), 0))
        db = QSqlDatabase.database(self.db_name)

        # Безопасный запрос с привязкой параметров (Аналог LINQ)
        ...
        # Прямое форматирование строки SQL (Аналог FromSqlRaw)
        query_raw = QSqlQuery(db)
        # Форматируем строку напрямую, подставляя employee_id
    ...
    def _set_employee_headers(self, model):
        headers = ["ID", "FullName", "Паспорт", "Должность", "Кол-во застрахованных"]
