...
func main() {
	...
	diagRepo := repository.NewDiagnosisRepository(db)
	diagSvc := service.NewDiagnosisService(diagRepo)
	diagHandler := handler.NewDiagnosisHandler(diagSvc)
	...
	api := r.Group("/api")
	{
		...
		protected.Use(middleware.AuthMiddleware())
		{
			...
			protected.GET("/diagnoses", diagHandler.GetAll)
