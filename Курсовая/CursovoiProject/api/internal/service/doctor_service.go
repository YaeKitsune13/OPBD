...
func (s *doctorService) CompleteVisit(appID uint, req dto.CompleteVisitRequest) error {
	...
	if err := s.repo.CompleteAppointment(appID, protocol); err != nil {
		return err
	}

	return s.repo.SetDiagnoses(appID, req.DiagnosisIDs)
}
...
func (s *doctorService) GetMedicalHistory(clientID uint) []dto.PetHistoryDTO {
	...
	for _, p := range pets {
		...
		for _, a := range p.Appointments {
			if a.Protocol != nil {
				diagnoses := make([]dto.DiagnosisDTO, 0, len(a.Diagnoses))
				for _, d := range a.Diagnoses {
					diagnoses = append(diagnoses, dto.DiagnosisDTO{ID: d.ID, Name: d.Name})
				}
